    ...
    methods: {
      handleSingleEdit () {
        if (this.$route.query.data) {
          ...
          if (data.IsEnableSync === true) {
            this.formValidate.serverSync = '启用'
          } else {
            this.formValidate.serverSync = '禁用'
          }
        }
      },
      handleAdd () {},
      handleUploadSuccess (res, file) {
        if (res.Code === 1) {
          ...
          this.imgUrl = this.apiUrl + res.Data
      ...
      handleSubmit (name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            netbarEdit(this.formValidate.startPath, '-1', this.formValidate.run, this.formValidate.name, this.formValidate.gamePath, this.iconPath, this.$route.query.data.Id, this.isSync).then((res) => {
