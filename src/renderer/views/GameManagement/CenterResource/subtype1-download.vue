      ...
      <Form :model="formItem" :label-width="80">
        ...
        <FormItem label="">
          <Row>
            ...
            <i-col span="4">
              <Select v-model="formItem.select2"  class="topColumn" style="width:150px;"   @on-change="handleSelect2Value">
                <Option v-for="item in serverList" :value="item.value" :key="item.value" placeholder="全部游戏类型">{{ item.value }}</Option>
              </Select>
            </i-col>
          </Row>
        </FormItem>
        <FormItem label="" v-if="this.showTable === true">
          <Row>
            <i-col span="2">同步服务器分配：</i-col>
            <i-col span="8">
        ...
        <FormItem>
          <Button type="primary" class="topColumn" @click="handleButtonDW">下载</Button>
          <Button class="topColumn" @click="handleButtonBack">返回</Button>
        </FormItem>
      </Form>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleSelect2Value (id) {
        var value = this.formItem.select2
        if (value !== '仅下载') {
          this.showTable = true
        } else {
